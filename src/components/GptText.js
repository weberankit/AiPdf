...
{selectSendDataMsg&&<div className="text-center text-xs">{selectSendDataMsg}</div>}
    ...
    { selectgeminiResponse.length && <div onClick={()=>sendDataToUser()} className="hover:cursor-pointer bg-red-700 rounded-lg text-white p-1 m-1 text-sm hover:bg-black parent">send <p className="element absolute top-[-30px]  bg-gray-700 p-2 z-[60] w-[600px]  left-0 text-xs"> send all AiSearch data  to your emailId</p></div>} 
  <p className="text-white text-sm font-semibold m-1 p-1 text-center">AiSearch </p>
       ...
       <div className="pt-2 mt-8 p-1 text-white">  <GeminiResponse/></div>
        </div>
