...
{selectSendDataMsg&&<div className="text-center text-md text-green-700 z-[2000] font-bold bg-[black] top-0  fixed  w-full  inline-block m-auto">{selectSendDataMsg}</div>}
    ...
    { selectgeminiResponse.length && <div onClick={()=>sendDataToUser()} className="hover:cursor-pointer bg-red-700 rounded-lg text-white p-1 m-1 text-sm hover:bg-black parent">send <p className="element absolute top-[40px]  bg-gray-700 p-2 z-[60] w-[600px]  left-0 text-xs"> send all data  to your emailId</p></div>} 
