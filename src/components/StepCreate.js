...
const StepCreate=({data,last,links})=>{
    return(
       ...
       <div className="text-sm font-semibold m-4 p-2"> If you face any difficulty for creating key <a href="mailto:impmessageweb@gmail.com" target="_blank" rel="noopener noreferrer"><button className="p-2 rounded-lg bg-black text-white hover:bg-yellow-400 text-xs">contact   </button></a> </div>
      <div className="pt-10">
       ...
       <div className="border border-yellow-700">  <span className="p-2 m-1 font-bold text-base animate-pulse">Step-10 : Note go to the <Link className="p-1 px-2 rounded-sm bg-black text-white" to={"/setting"}>Setting</Link> and follow beolw step</span><img src={last} alt="final-step " loading="lazy"></img></div>
        ...
        <p className="p-10 font-semibold text-sm">whenever if your key expires either you can purchase it or use other email-id to do this all above step</p>
        </>
    )
}
